...
function StorageLimit.unloadingStation_getFreeCapacity(station, superFunc, fillTypeIndex, farmId)
	...
	for _, targetStorage in pairs(station.targetStorages) do
        if farmId == nil or station:hasFarmAccessToStorage(farmId, targetStorage) then
			...
			for fillTypeIndex1, _ in pairs(targetStorage.fillTypes) do
				...
				if station:getFillLevel(fillTypeIndex1, farmId) > 0 and storedFillTypes[ftName] == nil then
	...
	local isFilltypeAlreadyInUse = station:getFillLevel(fillTypeIndex, farmId) > 0
	...
	if isFilltypeAlreadyInUse  then 
		...
		if countStoredFillTypes <= maxStoredFillTypesOverAll then
			if withOutput then
				...
				notificationText = string.format(g_i18n:getText("SiloCountFillTypeLimit_ExistingFiltype"), countStoredFillTypes, maxStoredFillTypesOverAll)
			end
		else
			notificationText = string.format(g_i18n:getText("SiloCountFillTypeLimit_UnloadingNotAllowed"), countStoredFillTypes, maxStoredFillTypesOverAll)
	...
	elseif countStoredFillTypes < maxStoredFillTypesOverAll then
		if withOutput then
			...
			notificationText = string.format(g_i18n:getText("SiloCountFillTypeLimit_NewFilltype"), countStoredFillTypes, maxStoredFillTypesOverAll)
		end
	else
		notificationText = string.format(g_i18n:getText("SiloCountFillTypeLimit_UnloadingNotAllowed"), countStoredFillTypes, maxStoredFillTypesOverAll)
		callSuperFunction = false
